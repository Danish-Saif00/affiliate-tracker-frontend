...
export function CompaniesPage() {
  ...
  const [statusUpdatingCompanyId, setStatusUpdatingCompanyId] =
  ...
  async function handleCompanyStatusChange(
    ...
    setStatusUpdatingCompanyId(companyId);

    try {
      await company.updateCompanyStatus(companyId, status);
      ...
      setStatusUpdatingCompanyId(null);
    }
  }
    ...
    <div className="companies-page page-stack">
      <GlassPanel as="section" className="page-heading-panel">
        <div>
          ...
          <h1>Companies</h1>
          <p>
            Create company workspaces, activate or suspend access, and choose the
            company whose administrators or subscription you want to manage.
          </p>
        </div>
                    ...
                    <div className="company-card__actions">
                      <button
                        ...
                        disabled={
                          selected ||
                          item.status !== 'active' ||
                          statusUpdatingCompanyId === item.id
                        }
                      ...
                      {item.status !== 'archived' && (
                        <button
                          className="glass-button company-status-button"
                          disabled={statusUpdatingCompanyId === item.id}
                          onClick={() =>
                            void handleCompanyStatusChange(
                              item.id,
