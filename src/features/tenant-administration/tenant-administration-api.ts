...
function parseDirectoryUser(value: unknown): CompanyDirectoryUser {
  ...
  return {
    ...
    invitedBy: readNullableString(value.invitedBy, "membership creator"),
...
function parseAuditEvent(value: unknown): AuditEvent {
  ...
  return {
    id: readRequiredString(value.id, "audit id"),
...
export async function createManagedUser(
  ...
  if (!isRecord(data)) {
    throw new Error("The API returned an invalid managed-user response.");
  }
...
export async function resetManagedUserPassword(
  ...
  const payload = await authenticatedApiRequest(
    accessToken,
    `/companies/${companyId}/managed-users/${input.userId}/password`,
    {
      method: "PATCH",
      companyId,
      body: {
  ...
  if (!isRecord(data)) {
    throw new Error("The API returned an invalid password-reset response.");
  }

  const passwordUpdated = readRequiredBoolean(
    data.passwordUpdated,
    "password-updated flag",
  );

  if (!passwordUpdated) {
    throw new Error("The API did not confirm the managed password reset.");
  }

  return {
    userId: readRequiredString(data.userId, "managed user id"),
