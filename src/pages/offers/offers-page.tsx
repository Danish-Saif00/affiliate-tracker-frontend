    ...
    <form className="catalog-form company-admin-offer-form" onSubmit={onSubmit}>
      <div className="catalog-form-section-heading">
        ...
        <div>
          ...
          <small>
            Company Admin assigns Offers to Managers. Managers later assign only
            these Offers to Publishers.
          </small>
        </div>
      </div>
      ...
      <div className="catalog-form-grid catalog-form-grid--three">
        ...
        <label>
          ...
          <select
            disabled={disabled}
            onChange={(event) =>
              onChange({ ...form, trackingDomainId: event.currentTarget.value })
            }
            required
        ...
        <label className="catalog-field--wide">
          <span>Promotional text template</span>
          <textarea
            disabled={disabled}
            maxLength={2000}
            onChange={(event) =>
              onChange({
                ...form,
                promotionalTextTemplate: event.currentTarget.value,
              })
            }
            required
            rows={4}
            value={form.promotionalTextTemplate}
          />
          <small>
            Supported placeholders: %OFFER_NAME%, %OFFER_ID%, %PUB_ID%,
            %COUNTRIES%, %DEVICES%, %PAYOUT%, and %TRACKING_LINK%.
          </small>
        </label>
        <label>
          <span>Status</span>
        ...
        {form.devices.includes("desktop") && (
          <label>
            ...
            <input
              disabled={disabled}
              onChange={(event) =>
                onChange({ ...form, desktopUrl: event.currentTarget.value })
              }
        ...
        {form.devices.includes("android") && (
          <label>
            ...
            <input
              disabled={disabled}
              onChange={(event) =>
                onChange({ ...form, androidUrl: event.currentTarget.value })
              }
        ...
        {form.devices.includes("ios") && (
          <label>
            ...
            <input
              disabled={disabled}
              onChange={(event) =>
                onChange({ ...form, iosUrl: event.currentTarget.value })
              }
      ...
      <div className="catalog-form-grid catalog-form-grid--three">
        ...
        <label>
          ...
          <select
            disabled={disabled}
            onChange={(event) =>
              onChange({ ...form, timezone: event.currentTarget.value })
            }
            value={form.timezone}
          >
        ...
        <label>
          ...
          <input
            ...
            min="0.01"
            onChange={(event) =>
              onChange({ ...form, payoutAmount: event.currentTarget.value })
            }
            placeholder="0.00"
            step="0.01"
            type="number"
        ...
        <label>
          ...
          <input
            disabled={disabled}
            onChange={(event) =>
              onChange({ ...form, expiresAt: event.currentTarget.value })
            }
            type="datetime-local"
      ...
      <div className="catalog-form-grid catalog-form-grid--two">
        <label>
          ...
          <input
            disabled={disabled}
            onChange={(event) =>
              onChange({ ...form, activeStartTime: event.currentTarget.value })
            }
            type="time"
        ...
        <label>
          ...
          <input
            disabled={disabled}
            onChange={(event) =>
              onChange({ ...form, activeEndTime: event.currentTarget.value })
            }
            type="time"
...
function updateInputFromOffer(
  ...
  if (offer.trackingDomainId === null) {
    throw new Error(
      "The Offer requires an active tracking Domain before its status can change.",
    );
  }
...
export function OffersPage({ mode }: { mode: OffersPageMode }) {
  ...
  const activeManagers = useMemo(
    () =>
      snapshot?.managers
        .filter(
          (item) =>
            item.membershipStatus === "active" && item.userStatus === "active",
        )
        .map((item) => ({
          membershipId: item.membershipId,
          publicId: item.publicId,
          label:
            item.displayName ?? item.email ?? item.membershipId.slice(0, 8),
        })) ?? [],
    [snapshot],
  );
  ...
  const editingOffer =
    editingId === null
      ? null
      : (snapshot?.offers.find((offer) => offer.id === editingId) ?? null);
  ...
  function createInput(current: OfferFormState): CreateCatalogOfferInput {
    ...
    const payoutAmount = amountToMinor(
      current.payoutAmount,
      current.payoutCurrency,
    );
    ...
    return {
      ...
      code:
        current.code.length > 0 ? current.code : createOfferCode(current.name),
      externalOfferId: current.externalOfferId.trim() || null,
      name: current.name,
      description: current.description.trim() || null,
      promotionalTextTemplate: current.promotionalTextTemplate,
    ...
    <div className="page-stack catalog-page company-admin-offers-page">
      <ControlModuleHeader
        ...
        stats={[
          ...
          {
            label: "Active",
            value: snapshot.offers.filter((offer) => offer.status === "active")
              .length,
          },
      ...
      {mode === "manage" && (
        <GlassPanel as="section" className="control-card catalog-table-panel control-directory-surface">
            ...
            >
              ...
              {snapshot.managers.map((manager) => (
                <option key={manager.membershipId} value={manager.membershipId}>
                  {manager.displayName ??
                    manager.email ??
                    manager.membershipId.slice(0, 8)}
                </option>
              ))}
            </select>
            <select
          ...
          {pageRows.length === 0 ? (
            ...
            <section
              className="offer-directory-cards"
                    ...
                    <div className="offer-directory-card__body">
                      ...
                      <dl className="offer-directory-card__details">
                        <div>
                          <dt>Domain</dt>
                          <dd>
                            {offer.trackingDomainHostname ?? "Not configured"}
                          </dd>
                        </div>
                        <div>
                          <dt>Countries</dt>
                        ...
                        <div>
                          <dt>Network</dt>
                          <dd>
                            {offer.networkAccountName} · {offer.providerName}
                          </dd>
                        </div>
                        <div>
                        ...
                        <div>
                          <dt>Conversions</dt>
                          <dd>{offer.conversions.toLocaleString()}</dd>
                        </div>
                        <div>
                          <dt>Timezone</dt>
                        ...
                        <div>
                          <dt>Active days</dt>
                          <dd>{offer.activeDays.join(", ")}</dd>
                        </div>
                        <div>
                          <dt>Active time</dt>
                          <dd>
                            {offer.activeStartTime === null
                              ? "All day"
                        ...
                        <div>
                          <dt>Expires</dt>
                          <dd>{offer.expiresAt ?? "No expiry"}</dd>
                        </div>
                        <div>
                          <dt>Proxy</dt>
                          <dd>{offer.proxyEnabled ? "Enabled" : "Disabled"}</dd>
                        </div>
                        <div>
                          <dt>Duplicates</dt>
                          <dd>
                            {offer.duplicateAllowed ? "Allowed" : "Blocked"}
                          </dd>
                        </div>
                      </dl>
                        ...
                        {catalog.permissions.canManageCatalog &&
                          ...
                          offer.status !== "archived" && (
                            <button
                              ...
                              onClick={() =>
                                void updateOfferStatus(offer, "active")
                              }
                              title="Activate"
                              type="button"
                            >
                        ...
                        {catalog.permissions.canManageCatalog &&
                          offer.status === "active" && (
                            <button
                              ...
                              onClick={() =>
                                void updateOfferStatus(offer, "paused")
                              }
                              title="Pause"
                              type="button"
                            >
