...
export function PublisherSessionsPage() {
  ...
  if (logs.status === "forbidden") {
    return (
      <ControlAccessDenied
        message="An active Publisher membership is required to view sessions."
    ...
    <div className="control-page final-operations-page">
      ...
      >
        <ControlCardHeading
          ...
          description={`${logs.sessions.length} session(s) match the current filters.`}
          eyebrow="Publisher Session Directory"
        ...
        {logs.sessions.length === 0 ? (
          <ControlEmpty
            icon="history"
            message="Sessions will appear when your tracking links receive traffic."
                ...
                <tbody>
                  {visibleRows.map((session) => (
                    <tr key={session.visitorId}>
                      <td>
                        <strong>{shortId(session.visitorId)}</strong>
                      </td>
                      <td title={session.ipHash}>
                        {privacyHash(session.ipHash)}
                      </td>
                      <td>{session.countryCode ?? "Not captured"}</td>
                      <td>{formatLabel(session.device)}</td>
                      <td>{session.browser}</td>
                      <td>
                        <strong>{session.clickCount}</strong>
                      </td>
