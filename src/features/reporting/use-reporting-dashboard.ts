import { useQuery } from "@tanstack/react-query";

import { useAuth } from "../auth/use-auth";
import { useCompany } from "../companies/use-company";
import {
  fetchOperationalEvents,
  fetchReportingDashboard,
} from "./reporting-api";
import type {
  CompanyReportingDashboard,
  OperationalEvent,
} from "./reporting.types";

export function useReportingDashboard(
  filters: { readonly from?: string; readonly to?: string } = {},
) {
  const auth = useAuth();
  const company = useCompany();
  const accessToken = auth.session?.access_token ?? null;
  const companyId = company.activeCompanyId;
  const platformRole = auth.identity?.authorization.platformRole ?? null;
  const membership = auth.identity?.authorization.companyMembership ?? null;
  const canReadOperationalEvents =
    platformRole === "platform_super_admin" ||
    (membership?.status === "active" &&
      (membership.role === "company_admin" || membership.role === "manager"));

  const dashboardQuery = useQuery<CompanyReportingDashboard>({
    queryKey: [
      "company-scoped",
      companyId,
      "reporting-dashboard",
      filters.from ?? null,
      filters.to ?? null,
    ],
    enabled: accessToken !== null && companyId !== null,
    queryFn: ({ signal }) => {
      if (accessToken === null || companyId === null) {
        throw new Error(
          "A selected company and authenticated session are required.",
        );
      }

      return fetchReportingDashboard(accessToken, companyId, filters, signal);
    },
  });

  const eventsQuery = useQuery<readonly OperationalEvent[]>({
    queryKey: ["company-scoped", companyId, "operational-events"],
    enabled:
      accessToken !== null && companyId !== null && canReadOperationalEvents,
    queryFn: ({ signal }) => {
      if (accessToken === null || companyId === null) {
        throw new Error(
          "A selected company and authenticated session are required.",
        );
      }

      return fetchOperationalEvents(accessToken, companyId, signal);
    },
  });

  return {
    dashboard: dashboardQuery.data ?? null,
    events: eventsQuery.data ?? [],
    error:
      dashboardQuery.error instanceof Error
        ? dashboardQuery.error.message
        : canReadOperationalEvents && eventsQuery.error instanceof Error
          ? eventsQuery.error.message
          : null,
    isLoading:
      dashboardQuery.isLoading ||
      (canReadOperationalEvents && eventsQuery.isLoading),
    refresh: async () => {
      await dashboardQuery.refetch();

      if (canReadOperationalEvents) {
        await eventsQuery.refetch();
      }
    },
  };
}
