...
import { fetchProxyConfiguration, updateProxyConfiguration } from "./proxy-api";
import type {
  CompanyProxyConfiguration,
  ProxyModuleLoadStatus,
  UpdateCompanyProxyConfigurationInput,
} from "./proxy.types";
const PROXY_QUERY_KEY = ["company-scoped", "proxy-configuration"] as const;
...
export function useProxyConfiguration() {
  ...
  const canManage =
    membership?.status === "active" && membership.role === "company_admin";
  const enabled = session !== null && companyId !== null && canManage;
  const query = useQuery<CompanyProxyConfiguration | null>({
    queryKey: [...PROXY_QUERY_KEY, companyId],
    enabled,
    queryFn: ({ signal }) => {
  ...
  >({
    mutationFn: async (input) => {
      if (session === null || companyId === null || !canManage) {
        throw new Error(
  ...
  return {
    ...
    status: resolveStatus(enabled, canManage, query.isLoading, query.isError),
    error: getErrorMessage(query.error ?? mutation.error),
    isMutating: mutation.isPending,
    updateConfiguration: mutation.mutateAsync,
