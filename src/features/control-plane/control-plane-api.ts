...
function parsePayoutProfile(value: unknown): PayoutProfile {
  ...
  if (
    !["invited", "active", "suspended", "revoked"].includes(membershipStatus)
  ) {
  ...
  return {
    ...
    createdAt: readRequiredString(
      value.createdAt,
      "payout profile creation time",
    ),
    updatedAt: readRequiredString(
      value.updatedAt,
      "payout profile update time",
    ),
  };
}
...
function parseOfferAssignment(value: unknown): OfferAssignment {
  ...
  const payoutMode = readRequiredString(
    value.payoutMode,
    "assignment payout mode",
  );
  ...
  if (
    ...
    throw new Error(
      "The API returned an unsupported assignment membership status.",
    );
  }
...
function parseTrackingLink(value: unknown): TrackingLink {
  ...
  const ownerRole = readRequiredString(
    value.ownerRole,
    "tracking link owner role",
  );
  ...
  if (!["manager", "publisher"].includes(ownerRole)) {
    throw new Error(
      "The API returned an unsupported tracking-link owner role.",
    );
  }

  if (
    !["invited", "active", "suspended", "revoked"].includes(
      ownerMembershipStatus,
    )
  ) {
    throw new Error(
      "The API returned an unsupported tracking-link membership status.",
    );
  }
  ...
  return {
    ...
    trackingDomainId: readRequiredString(
      value.trackingDomainId,
      "tracking link domain id",
    ),
    ...
    ownerMembershipId: readRequiredString(
      ...
      "tracking link owner membership id",
    ),
    ownerUserId: readRequiredString(
      value.ownerUserId,
      "tracking link owner user id",
    ),
    ownerRole: ownerRole as TrackingLink["ownerRole"],
    ownerMembershipStatus:
      ownerMembershipStatus as TrackingLink["ownerMembershipStatus"],
    trackingCode: readRequiredString(value.trackingCode, "tracking code"),
    customSlug: readNullableString(
      value.customSlug,
      "tracking link custom slug",
    ),
    destinationUrl: readRequiredString(
      value.destinationUrl,
    ...
    source: source as TrackingLink["source"],
    ...
    createdAt: readRequiredString(
      value.createdAt,
      "tracking link creation time",
    ),
    updatedAt: readRequiredString(value.updatedAt, "tracking link update time"),
  };
}

function parseDeleteTrackingLinkResult(
  value: unknown,
): DeleteTrackingLinkResult {
  if (!isRecord(value)) {
    throw new Error("The API returned an invalid tracking-link delete result.");
  }

  const id = readRequiredString(value.id, "tracking-link ID");
...
function parsePostbackEndpoint(value: unknown): NetworkPostbackEndpoint {
  ...
  if (!["active", "paused", "archived"].includes(status)) {
    throw new Error(
      "The API returned an unsupported postback endpoint status.",
    );
  }
  ...
  return {
    ...
    companyId: readRequiredString(
      value.companyId,
      "postback endpoint company id",
    ),
    networkAccountId: readRequiredString(
      value.networkAccountId,
      "postback network account id",
    ),
    networkAccountName: readRequiredString(
      value.networkAccountName,
      "postback network account name",
    ),
    name: readRequiredString(value.name, "postback endpoint name"),
    endpointKeyLast4: readRequiredString(
      value.endpointKeyLast4,
      "postback endpoint key suffix",
    ),
    ...
    createdAt: readRequiredString(
      value.createdAt,
      "postback endpoint creation time",
    ),
    updatedAt: readRequiredString(
      value.updatedAt,
      "postback endpoint update time",
    ),
  };
}

function parsePostbackEndpointSetup(
  value: unknown,
): NetworkPostbackEndpointSetup {
  if (!isRecord(value)) {
    throw new Error("The API returned an invalid Provider postback setup.");
  }

  return {
    providerId: readRequiredString(value.providerId, "postback Provider id"),
    providerCode: readRequiredString(
      value.providerCode,
      "postback Provider code",
    ),
    providerName: readRequiredString(
      value.providerName,
      "postback Provider name",
    ),
    effectiveTrackingParameter: readRequiredString(
      value.effectiveTrackingParameter,
      "effective tracking parameter",
    ),
    basePath: readRequiredString(value.basePath, "postback base path"),
    templatePath: readNullableString(
      value.templatePath,
      "Provider postback template path",
    ),
    integrationConfigured: readBoolean(
      value.integrationConfigured,
      "Provider integration configured flag",
    ),
  };
}

function parsePostbackEndpointSecret(
  value: unknown,
): NetworkPostbackEndpointSecret {
  if (!isRecord(value)) {
  ...
  return {
    ...
    setup: parsePostbackEndpointSetup(value.setup),
  };
}
...
function parseConversion(value: unknown): Conversion {
  ...
  if (!["provider_postback", "manual"].includes(source)) {
  ...
  return {
    ...
    trackingClickId: readRequiredString(
      value.trackingClickId,
      "tracking click id",
    ),
    publicClickId: readRequiredString(value.publicClickId, "public click id"),
    trackingLinkId: readRequiredString(
      value.trackingLinkId,
      "conversion tracking link id",
    ),
    ...
    networkAccountId: readRequiredString(
      value.networkAccountId,
      "conversion network account id",
    ),
    networkAccountName: readRequiredString(
      value.networkAccountName,
      "conversion network account name",
    ),
    ownerMembershipId: readRequiredString(
      value.ownerMembershipId,
      "conversion owner membership id",
    ),
    ownerUserId: readRequiredString(
      value.ownerUserId,
      "conversion owner user id",
    ),
    offerAssignmentId: readRequiredString(
      value.offerAssignmentId,
      "conversion assignment id",
    ),
    postbackEndpointId: readNullableString(
      value.postbackEndpointId,
      "conversion endpoint id",
    ),
    postbackEndpointName: readNullableString(
      value.postbackEndpointName,
      "conversion endpoint name",
    ),
    externalConversionId: readRequiredString(
      value.externalConversionId,
      "external conversion id",
    ),
    source: source as Conversion["source"],
    status: status as Conversion["status"],
    revenueAmountMinor: readOptionalNumber(
      value.revenueAmountMinor,
      "conversion revenue amount",
    ),
    revenueCurrency: readNullableString(
      value.revenueCurrency,
      "conversion revenue currency",
    ),
    payoutMode: payoutMode as Conversion["payoutMode"],
    payoutAmountMinor: readRequiredNumber(
      value.payoutAmountMinor,
      "conversion payout amount",
    ),
    payoutCurrency: readRequiredString(
      value.payoutCurrency,
      "conversion payout currency",
    ),
...
function parseDuplicateRule(value: unknown): DuplicateProtectionRule {
  ...
  return {
    ...
    networkAccountId: readRequiredString(
      value.networkAccountId,
      "duplicate rule network account id",
    ),
    networkAccountName: readRequiredString(
      value.networkAccountName,
      "duplicate rule network account name",
    ),
    ...
    sessionWindowSeconds: readOptionalNumber(
      value.sessionWindowSeconds,
      "session window",
    ),
    lockDurationSeconds: readOptionalNumber(
      value.lockDurationSeconds,
      "lock duration",
    ),
    ...
    matchIpAndUserAgent: readBoolean(
      value.matchIpAndUserAgent,
      "IP and user-agent matching flag",
    ),
    rapidRepeatWindowSeconds: readRequiredNumber(
      value.rapidRepeatWindowSeconds,
      "rapid-repeat window",
    ),
    rapidRepeatThreshold: readRequiredNumber(
      value.rapidRepeatThreshold,
      "rapid-repeat threshold",
    ),
    ...
    createdAt: readRequiredString(
      value.createdAt,
      "duplicate rule creation time",
    ),
    updatedAt: readRequiredString(
      value.updatedAt,
      "duplicate rule update time",
    ),
  };
}
...
function parseFraudClick(value: unknown): FraudClick {
  ...
  const fraudRiskLevel = readRequiredString(
  ...
  return {
    ...
    trackingLinkId: readRequiredString(
      value.trackingLinkId,
      "fraud click tracking link id",
    ),
    offerId: readRequiredString(value.offerId, "fraud click offer id"),
    networkAccountId: readRequiredString(
      value.networkAccountId,
      "fraud click network account id",
    ),
    ownerMembershipId: readRequiredString(
      value.ownerMembershipId,
      "fraud click owner membership id",
    ),
    ownerUserId: readRequiredString(
      value.ownerUserId,
      "fraud click owner user id",
    ),
    ...
    duplicateReason: readNullableString(
      value.duplicateReason,
      "duplicate reason",
    ),
    duplicateOfClickId: readNullableString(
      value.duplicateOfClickId,
      "original click id",
    ),
    duplicateRuleId: readNullableString(
      value.duplicateRuleId,
      "duplicate rule id",
    ),
    lockExpiresAt: readNullableString(
      value.lockExpiresAt,
      "duplicate lock expiration",
    ),
    ...
    attributionEligible: readBoolean(
      value.attributionEligible,
      "attribution eligibility",
    ),
    capturedAt: readRequiredString(
      value.capturedAt,
      "fraud click capture time",
    ),
  };
}
...
function parseBillingEntitlement(value: unknown): BillingPlanEntitlement {
  ...
  return {
    ...
    limitValue: readOptionalNumber(
      value.limitValue,
      "billing entitlement limit",
    ),
    createdAt: readRequiredString(
      value.createdAt,
      "billing entitlement creation time",
    ),
    updatedAt: readRequiredString(
      value.updatedAt,
      "billing entitlement update time",
    ),
  };
}
...
function parseBillingPlan(value: unknown): BillingPlan {
  ...
  const billingInterval = readRequiredString(
  ...
  return {
    ...
    priceAmountMinor: readRequiredNumber(
    ...
    gracePeriodDays: readRequiredNumber(
      value.gracePeriodDays,
      "billing plan grace days",
    ),
    createdBy: readNullableString(value.createdBy, "billing plan creator"),
    createdAt: readRequiredString(
      value.createdAt,
      "billing plan creation time",
    ),
    updatedAt: readRequiredString(value.updatedAt, "billing plan update time"),
    entitlements: readArray(value.entitlements, "billing entitlements").map(
...
function parseBillingSnapshot(value: unknown): CompanyBillingSnapshot {
  ...
  const companyStatus = readRequiredString(
    value.companyStatus,
    "billing company status",
  );
  ...
  if (subscriptionValue !== null) {
    ...
    subscription = {
      ...
      companyId: readRequiredString(
        subscriptionValue.companyId,
        "subscription company id",
      ),
      planId: readRequiredString(
        subscriptionValue.planId,
        "subscription plan id",
      ),
      status: status as NonNullable<
        CompanyBillingSnapshot["subscription"]
      >["status"],
      startsAt: readRequiredString(
        subscriptionValue.startsAt,
        "subscription start",
      ),
      trialEndsAt: readNullableString(
        subscriptionValue.trialEndsAt,
      ...
      graceEndsAt: readNullableString(
        subscriptionValue.graceEndsAt,
        "subscription grace end",
      ),
      canceledAt: readNullableString(
        subscriptionValue.canceledAt,
        "subscription cancellation time",
      ),
      endedAt: readNullableString(
        subscriptionValue.endedAt,
      ...
      createdBy: readNullableString(
        subscriptionValue.createdBy,
        "subscription creator",
      ),
      updatedBy: readNullableString(
        subscriptionValue.updatedBy,
  ...
  return {
    ...
    access: {
      ...
      effectiveUntil: readNullableString(
        value.access.effectiveUntil,
...
function parsePerformanceRow(value: unknown): ReportingPerformanceRow {
  ...
  return {
    dimensionId: readRequiredString(
      value.dimensionId,
      "reporting dimension id",
    ),
    dimensionName: readRequiredString(
      value.dimensionName,
      "reporting dimension name",
    ),
    clicks: readRequiredNumber(value.clicks, "reporting click count"),
    conversions: readRequiredNumber(
      value.conversions,
      "reporting conversion count",
    ),
...
function parseReportingDashboard(value: unknown): ReportingDashboard {
  ...
  return {
    ...
    totals: {
      ...
      uniqueVisitors: readRequiredNumber(
        value.totals.uniqueVisitors,
        "reporting unique visitors",
      ),
      duplicateClicks: readRequiredNumber(
        value.totals.duplicateClicks,
        "reporting duplicate clicks",
      ),
      highRiskClicks: readRequiredNumber(
        value.totals.highRiskClicks,
        "reporting high-risk clicks",
      ),
      conversions: readRequiredNumber(
    ...
    offers: readArray(value.offers, "offer performance").map(
      parsePerformanceRow,
    ),
    ...
    members: readArray(value.members, "member performance").map(
      parsePerformanceRow,
    ),
  };
}
...
function parseOperationalEvent(value: unknown): OperationalEvent {
  ...
  return {
    ...
    companyId: readNullableString(
      value.companyId,
      "operational event company id",
    ),
    actorUserId: readNullableString(
      value.actorUserId,
      "operational event actor id",
    ),
    requestId: readNullableString(
      value.requestId,
      "operational event request id",
    ),
    ...
    createdAt: readRequiredString(
      value.createdAt,
      "operational event creation time",
    ),
  };
}
...
function parseCustomization(value: unknown): CompanyCustomization {
  ...
  const linkIdentifierMode = readRequiredString(
  ...
  if (
    ...
    throw new Error("The API returned an unsupported link identifier mode.");
  }
  const restrictedSharePlatforms = readArray(
    value.restrictedSharePlatforms,
    "restricted share platforms",
  ).map((item) => {
    const platform = readRequiredString(item, "restricted share platform");
    if (
      platform !== "snapchat" &&
      platform !== "instagram" &&
      platform !== "facebook"
    ...
    return platform as CompanyCustomization["restrictedSharePlatforms"][number];
  });
  return {
    id: readRequiredString(value.id, "customization id"),
    companyId: readRequiredString(value.companyId, "customization company id"),
    brandName: readNullableString(value.brandName, "custom brand name"),
    tagline: readNullableString(value.tagline, "custom tagline"),
    logoUrl: readNullableString(value.logoUrl, "custom logo URL"),
    ...
    linkIdentifierMode:
      linkIdentifierMode as CompanyCustomization["linkIdentifierMode"],
    plainTextSharingEnabled: readBoolean(
      value.plainTextSharingEnabled,
      "plain-text sharing flag",
    ),
    restrictedSharePlatforms,
    defaultLinkQueryParameters: readStringRecord(
      value.defaultLinkQueryParameters,
    ),
    createdBy: readNullableString(value.createdBy, "customization creator"),
    updatedBy: readNullableString(value.updatedBy, "customization updater"),
    createdAt: readRequiredString(
      value.createdAt,
      "customization creation time",
    ),
...
function parseNullableCompanyCustomization(
  ...
  return value === null ? null : parseCustomization(value);
}
...
export async function fetchOfferAssignments(
  ...
  return readArray(readData(payload), "offer assignments").map(
    parseOfferAssignment,
  );
}
  ...
  const payload = await authenticatedApiRequest(
    ...
    `/companies/${companyId}/offers/${input.offerId}/assignments`,
    {
      ...
      body: {
        ...
        manualPayoutCurrency:
          input.manualPayoutCurrency?.trim().toUpperCase() || null,
      },
    },
  );
...
export async function fetchTrackingLinks(
  ...
  return readArray(
    readNestedData(payload, "trackingLinks"),
    "tracking links",
  ).map(parseTrackingLink);
}
    ...
    `/companies/${companyId}/tracking-links`,
    {
      ...
      body: {
        ...
        ...(input.customSlug !== undefined && input.customSlug.trim().length > 0
          ...
          : {}),
        ...(input.destinationUrl !== undefined &&
        input.destinationUrl.trim().length > 0
          ? { destinationUrl: input.destinationUrl.trim() }
          : {}),
...
export async function cloneTrackingLink(
  accessToken: string,
  companyId: string,
  linkId: string,
): Promise<TrackingLink> {
...
export async function archiveTrackingLink(
  accessToken: string,
  companyId: string,
  linkId: string,
): Promise<TrackingLink> {
...
export async function deleteTrackingLink(
  ...
  const payload = await authenticatedApiRequest(
    accessToken,
    `/companies/${companyId}/tracking-links/${linkId}`,
    {
      method: "DELETE",
      companyId,
    },
  ...
  return parseDeleteTrackingLinkResult(readData(payload));
}
...
export async function fetchPostbackEndpoints(
  ...
  return readArray(
    readNestedData(payload, "endpoints"),
    "postback endpoints",
  ).map(parsePostbackEndpoint);
}
...
export async function fetchConversions(
  ...
  return readArray(readNestedData(payload, "conversions"), "conversions").map(
    parseConversion,
  );
}
...
export async function fetchDuplicateRules(
  ...
  return readArray(
    readNestedData(payload, "rules"),
    "duplicate-protection rules",
  ).map(parseDuplicateRule);
}
...
export async function fetchFraudClicks(
  ...
  return readArray(readNestedData(payload, "clicks"), "fraud clicks").map(
    parseFraudClick,
  );
}
...
export async function fetchCustomization(
  ...
  return parseNullableCompanyCustomization(
    readNestedData(payload, "customization"),
  );
}
