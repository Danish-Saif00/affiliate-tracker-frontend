...
export function BillingPage() {
  ...
  async function handleRenewSubscription(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setFeedback(null);
    setActionError(null);
    ...
    try {
      const form = event.currentTarget;
      const data = new FormData(form);
      const currentPeriodEndsAt = optionalDate(data.get('renewalEndsAt'));

      if (currentPeriodEndsAt === undefined) {
        throw new Error('A future subscription end date is required.');
      }

      const snapshot = await billing.updateSubscription({
        status: 'active',
        currentPeriodEndsAt,
      });
      ...
      form.reset();
      setFeedback(
        `${billing.companyName} access was renewed until ${formatDateTime(snapshot.subscription?.currentPeriodEndsAt ?? currentPeriodEndsAt)}.`,
      );
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'Subscription renewal failed.');
    }
  }
                  ...
                  {billing.permissions.canManagePlatform && (
                    <>
                      <form
                        ...
                        onSubmit={(event) => void handleRenewSubscription(event)}
                      >
                        <label className="control-inline-field">
                          <span>Renew or extend access until</span>
                          <input
